...
- 🔨 **Auction Scanner** — live in production at [scanner.auctionit.io](https://scanner.auctionit.io) *(source private)*. Staff photograph an estate sale, the app groups multi-angle photos into items, identifies each one via reverse image search, and pulls market pricing evidence — but the human sets the final price on every item, not the model. FastAPI + React on Render, Postgres, Cloudflare R2. Phase 2A (multi-user admin, cost tracking) shipped as v1.2.0; measured cost: $1.15 for a 20-photo batch.
